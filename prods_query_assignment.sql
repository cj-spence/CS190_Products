...
SELECT cust_id, invoice_num FROM invoice
...
SELECT prod_id, prod_desc, (prod_price * prod_quantity) AS TOTAL_VALUE FROM product
WHERE (prod_price * prod_quantity) >= '4000';
...
SELECT cust_name, cust_id, cust_balance, cust_limit FROM customer
...
SELECT cust_name, cust_id, cust_balance FROM customer
